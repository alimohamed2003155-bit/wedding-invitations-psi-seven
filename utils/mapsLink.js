...
/**
 * بيتأكد إن الـ host فعلاً دومين جوجل حقيقي (مش أي دومين حاطط كلمة google
 * أو maps جواه). ده اللي بيمنع حد يلصق رابط زي
 * "https://evil.com/x/maps/embed?y=..." أو "https://google.com.evil.net/..."
 * ويخلينا نحطه كـ iframe src أو رابط "افتح في الخرائط" على دعوة حقيقية.
 * @param {string} host - hostname بعد شيل www. (زي ما بيرجعه new URL().hostname)
 * @returns {boolean}
 */
function isTrustedGoogleMapsHost(host) {
  if (SHORT_LINK_HOSTS.includes(host)) return true;
  // لازم يخلص بالظبط بـ ".google.<tld>" أو يكون "google.<tld>" نفسه —
  // مش مجرد يحتوي على كلمة google في أي حتة من الاسم
  return /^(?:[a-z0-9-]+\.)*google\.[a-z.]{2,}$/i.test(host);
}
...
async function resolveMapInput({ raw, venueName, venueCity, skipNetwork }) {
  ...
  // "الرابط المباشر" (زرار "افتح في الخرائط") ميتوثقش إلا لو فعلاً دومين
  // جوجل — غير كده بيبقى نفس النص المُدخل لكن كرابط بحث آمن على جوجل مابس،
  // عشان محدش يقدر يحط رابط تصيّد وراه لابل "Google Maps"
  const safeDirectLink = isTrustedGoogleMapsHost(host)
    ? trimmed
    : textEmbed(fallbackText || trimmed).directLink;

  // لينكات مضمّنة جاهزة من الأساس (نادر، بس لو حصل نستخدمها زي ما هي) —
  // بس لازم يكون فعلاً دومين جوجل، مش أي دومين حاطط "/maps/embed" في اللينك
  if (isTrustedGoogleMapsHost(host) && workingUrl.includes('/maps/embed')) {
    return { embedSrc: workingUrl, directLink: safeDirectLink };
  }
  ...
  if (coords) {
    return coordsEmbed(coords.lat, coords.lng, safeDirectLink);
  }

  const placeName = extractPlaceNameFromUrl(workingUrl);
  if (placeName) {
    return { embedSrc: textEmbed(placeName).embedSrc, directLink: safeDirectLink };
  }
  ...
  return { embedSrc: textEmbed(fallbackText || trimmed).embedSrc, directLink: safeDirectLink };
}

module.exports = { resolveMapInput, isUrl, extractCoordsFromUrl, extractPlaceNameFromUrl, isTrustedGoogleMapsHost };
