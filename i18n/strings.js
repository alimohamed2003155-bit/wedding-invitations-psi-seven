// i18n/strings.js
// كل النصوص الثابتة في التصميم (اللي مش اسم عروسة/عريس/تاريخ) بيتم سحبها من هنا.
// أي قالب جديد تضيفه في المستقبل يقدر يستخدم نفس القاموس ده.

const STRINGS = {
  ar: {
    tapToOpen: 'اضغط للفتح',
    heroEyebrow: { wedding: 'يوم الزفاف', engagement: 'يوم الخطوبة' },
    celebrationBegins: { wedding: 'يبدأ الاحتفال', engagement: 'يبدأ حفل الخطوبة' },
    countdown: { days: 'أيام', hours: 'ساعات', minutes: 'دقايق', seconds: 'ثواني' },
    venueTitle: 'المكان',
    timelineTitle: { wedding: 'برنامج حفل الزفاف', engagement: 'برنامج حفل الخطوبة' },
    timelineStages: {
      reception: 'الاستقبال',
      ceremony: { wedding: 'عقد القران', engagement: 'مراسم الخطوبة' },
      cocktail: 'حفل الكوكتيل',
      dinner: 'العشاء',
      party: 'الحفلة',
      farewell: 'ختام السهرة',
    },
    heroTagline: { wedding: 'يتزوّجان!', engagement: 'يخطبان!' },
    dressCodeIntro: 'يسعدنا لو كان اختياركم لملابس السهرة متناسقًا مع ألوان الفرح.',
    scratchSectionTitle: 'التاريخ',
    scratchInstruction: '✦ اخدشوا لتظهر لكم التاريخ ✦',
    scratchDayLabel: 'اليوم',
    scratchMonthLabel: 'الشهر',
    scratchYearLabel: 'السنة',
    scratchRevealMessage: { wedding: 'أنتم مدعوون!', engagement: 'أنتم مدعوون!' },
    rsvpSubmittingText: 'جارِ الإرسال...',
    rsvpSuccessText: 'تم استلام ردكم، شكرًا لكم!',
    rsvpErrorText: 'حصل خطأ، من فضلكم حاولوا تاني.',
    dressCodeTitle: 'الزي المقترح',
    colorPalette: 'الألوان المقترحة',
    ladies: 'السيدات',
    gentlemen: 'السادة',
    ladiesDesc: 'يُفضَّل فستان سهرة أنيق بألوان راقية.',
    gentlemenDesc: 'يُفضَّل بدلة رسمية كلاسيكية مع حذاء أنيق.',
    rsvpTitle: 'تأكيد الحضور',
    rsvpIntro: 'عشان نقدر نجهز لاحتفال يليق بيكم، محتاجين تأكيد حضوركم.',
    rsvpButtonText: 'أكّد حضورك',
    rsvpDeadlineNote: 'من فضلكم أكّدوا حضوركم في أقرب وقت ممكن.',
    yourNameLabel: 'اسمك',
    willYouComeLabel: 'هتحضروا؟',
    yesOption: 'أكيد هنكون موجودين',
    noOption: 'للأسف مش هنقدر نحضر',
    foodIntoleranceLabel: 'في حساسية معينة من أي أكل؟',
    submitButtonText: 'إرسال',
    mapTitle: 'الموقع على خرائط جوجل',
    openInMapsText: 'افتح في خرائط جوجل',
    closingLine: 'يسعدنا ويشرفنا حضوركم',
    openingTitle: { wedding: 'أعزاءنا الأهل والأصدقاء،', engagement: 'أعزاءنا الأهل والأصدقاء،' },
    openingBody: {
      wedding: 'وإحنا بنستعد نقول "قبلت"، حاسّين بالامتنان لوجود ناس طيبين زيكم في حياتنا.<br><br>وجودكم معانا هيكون أكبر هدية، ووجودكم هيشرّفنا ونحنا بنبدأ حياتنا الجديدة مع بعض.',
      engagement: 'وإحنا بنستعد نبدأ الفصل الجديد ده في حياتنا، حاسّين بالامتنان لوجود ناس طيبين زيكم في حياتنا.<br><br>وجودكم معانا في حفل خطوبتنا هيكون أكبر هدية لينا.',
    },
    detailsTitle: 'تفاصيل إضافية',
    contactIntro: 'لأي استفسار أو تفاصيل إضافية، تقدروا تتواصلوا مع منظمين الحفل.',
    giftNote: 'وجودكم هو أكبر هدية لينا. لو حابين تكرمونا بهدية، أي مساهمة في بداية حياتنا هتبقى موضع تقدير كبير.',
  },

  en: {
    tapToOpen: 'Tap to open',
    heroEyebrow: { wedding: 'Wedding Day', engagement: 'Engagement Day' },
    celebrationBegins: { wedding: 'The Celebration Begins', engagement: 'The Engagement Begins' },
    countdown: { days: 'Days', hours: 'Hours', minutes: 'Minutes', seconds: 'Seconds' },
    venueTitle: 'Venue',
    timelineTitle: { wedding: 'Wedding Day Timeline', engagement: 'Engagement Day Timeline' },
    timelineStages: {
      reception: 'Welcome Reception',
      ceremony: { wedding: 'Nikah Ceremony', engagement: 'Engagement Ceremony' },
      cocktail: 'Cocktail Hour',
      dinner: 'Dinner',
      party: 'Party',
      farewell: 'End of the Celebration',
    },
    heroTagline: { wedding: 'are getting married!', engagement: 'are getting engaged!' },
    dressCodeIntro: 'We would be very happy if your outfit is in the colours of the wedding theme.',
    scratchSectionTitle: 'The Date',
    scratchInstruction: '✦ Scratch to reveal the date ✦',
    scratchDayLabel: 'Day',
    scratchMonthLabel: 'Month',
    scratchYearLabel: 'Year',
    scratchRevealMessage: { wedding: 'You\'re Invited!', engagement: 'You\'re Invited!' },
    rsvpSubmittingText: 'Submitting...',
    rsvpSuccessText: 'Thank you! Your response has been received.',
    rsvpErrorText: 'Something went wrong, please try again.',
    dressCodeTitle: 'Dress Code',
    colorPalette: 'Color palette',
    ladies: 'Ladies',
    gentlemen: 'Gentlemen',
    ladiesDesc: 'Formal dresses in elegant, polished styles are encouraged.',
    gentlemenDesc: 'Well-tailored suits with classic dress shoes are preferred.',
    rsvpTitle: 'Confirm Your Presence',
    rsvpIntro: 'To help us prepare for a joyful celebration, kindly confirm your attendance.',
    rsvpButtonText: 'Confirm Your Attendance',
    rsvpDeadlineNote: 'Please confirm your attendance as soon as possible.',
    yourNameLabel: 'Your name',
    willYouComeLabel: 'Will you come?',
    yesOption: 'Yes, I will be there',
    noOption: 'Unfortunately, I can\'t make it',
    foodIntoleranceLabel: 'Do you have any food intolerances?',
    submitButtonText: 'Submit',
    mapTitle: 'Google Maps Directions',
    openInMapsText: 'Open in Google Maps',
    closingLine: 'We would be delighted to welcome you',
    openingTitle: { wedding: 'Dear Friends and Family,', engagement: 'Dear Friends and Family,' },
    openingBody: {
      wedding: 'As we get ready to say &ldquo;I do,&rdquo; we feel grateful for the wonderful people in our lives.<br><br>Your support means the world to us, and we would be honored to have you with us as we begin our life together.',
      engagement: 'As we get ready to begin this new chapter, we feel grateful for the wonderful people in our lives.<br><br>Your support means the world to us, and we would be honored to have you with us as we celebrate our engagement.',
    },
    detailsTitle: 'Details',
    contactIntro: 'For additional information or questions, please contact the wedding organizers.',
    giftNote: 'Your presence is the greatest gift to us. However, if you wish to honor us with a present, a contribution toward our future would be sincerely appreciated.',
  },

  fr: {
    tapToOpen: 'Appuyez pour ouvrir',
    heroEyebrow: { wedding: 'Jour du Mariage', engagement: 'Jour des Fiançailles' },
    celebrationBegins: { wedding: 'La Célébration Commence', engagement: 'Les Fiançailles Commencent' },
    countdown: { days: 'Jours', hours: 'Heures', minutes: 'Minutes', seconds: 'Secondes' },
    venueTitle: 'Lieu',
    timelineTitle: { wedding: 'Chronologie de l\'événement', engagement: 'Chronologie des fiançailles' },
    timelineStages: {
      reception: 'Réception d\'Accueil',
      ceremony: { wedding: 'Cérémonie de Mariage', engagement: 'Cérémonie de fiançailles' },
      cocktail: 'Heure du Cocktail',
      dinner: 'Dîner',
      party: 'Soirée Dansante',
      farewell: 'Fin de la Soirée',
    },
    heroTagline: { wedding: 'se marient !', engagement: 'se fiancent !' },
    dressCodeIntro: 'Nous serions ravis que votre tenue s\'accorde aux couleurs du mariage.',
    scratchSectionTitle: 'La Date',
    scratchInstruction: '✦ Grattez pour révéler la date ✦',
    scratchDayLabel: 'Jour',
    scratchMonthLabel: 'Mois',
    scratchYearLabel: 'Année',
    scratchRevealMessage: { wedding: 'Vous êtes invités !', engagement: 'Vous êtes invités !' },
    rsvpSubmittingText: 'Envoi en cours...',
    rsvpSuccessText: 'Merci ! Votre réponse a bien été reçue.',
    rsvpErrorText: 'Une erreur est survenue, merci de réessayer.',
    dressCodeTitle: 'Tenue de Soirée',
    colorPalette: 'Palette de couleurs',
    ladies: 'Mesdames',
    gentlemen: 'Messieurs',
    ladiesDesc: 'Des robes de soirée élégantes et raffinées sont recommandées.',
    gentlemenDesc: 'Costumes bien ajustés avec chaussures habillées classiques de préférence.',
    rsvpTitle: 'Confirmez Votre Présence',
    rsvpIntro: 'Afin de préparer au mieux cette célébration, merci de confirmer votre présence.',
    rsvpButtonText: 'Confirmez Votre Présence',
    rsvpDeadlineNote: 'Merci de confirmer votre présence dans les plus brefs délais.',
    yourNameLabel: 'Votre nom',
    willYouComeLabel: 'Serez-vous présent(e) ?',
    yesOption: 'Oui, je serai présent(e)',
    noOption: 'Malheureusement, je ne pourrai pas venir',
    foodIntoleranceLabel: 'Avez-vous des intolérances alimentaires ?',
    submitButtonText: 'Envoyer',
    mapTitle: 'Itinéraire Google Maps',
    openInMapsText: 'Ouvrir dans Google Maps',
    closingLine: 'Au plaisir de vous accueillir',
    openingTitle: { wedding: 'Chers amis et famille,', engagement: 'Chers amis et famille,' },
    openingBody: {
      wedding: 'Alors que nous nous préparons à dire &laquo;oui&raquo;, nous sommes reconnaissants d\'avoir des personnes merveilleuses comme vous dans nos vies.<br><br>Votre soutien compte énormément pour nous, et ce serait un honneur de vous avoir à nos côtés pour le début de notre vie commune.',
      engagement: 'Alors que nous nous préparons à commencer ce nouveau chapitre, nous sommes reconnaissants d\'avoir des personnes merveilleuses comme vous dans nos vies.<br><br>Votre soutien compte énormément pour nous, et ce serait un honneur de vous avoir à nos côtés pour célébrer nos fiançailles.',
    },
    detailsTitle: 'Détails',
    contactIntro: 'Pour toute information complémentaire, merci de contacter les organisateurs.',
    giftNote: 'Votre présence est le plus beau des cadeaux. Toutefois, si vous souhaitez nous honorer d\'un présent, une contribution pour notre avenir serait sincèrement appréciée.',
  },
};

// الأول في القايمة = الافتراضي لأي دعوة جديدة (مجانية أو مدفوعة)،
// وكمان ترتيب ظهور الاختيارات في الفورم. الإنجليزي هو الافتراضي.
// الدعوات القديمة بتخزّن لغتها صراحةً، فترتيب القايمة هنا مبيأثرش عليها.
const SUPPORTED_LANGUAGES = ['en', 'ar', 'fr'];
const SUPPORTED_OCCASIONS = ['wedding', 'engagement'];

/**
 * بيرجع نسخة "مسطّحة" من القاموس لغة معينة ومناسبة معينة، جاهزة للحقن في
 * التصميم من غير ما الملف اللي بيستخدمها يحتاج يعرف تفاصيل التركيب الداخلي.
 */
function getStrings(language, occasionType) {
  const lang = SUPPORTED_LANGUAGES.includes(language) ? language : 'ar';
  const occ = SUPPORTED_OCCASIONS.includes(occasionType) ? occasionType : 'wedding';
  const s = STRINGS[lang];

  return {
    tapToOpen: s.tapToOpen,
    heroEyebrow: s.heroEyebrow[occ],
    celebrationBegins: s.celebrationBegins[occ],
    countdown: s.countdown,
    venueTitle: s.venueTitle,
    timelineTitle: s.timelineTitle[occ],
    timelineStages: {
      reception: s.timelineStages.reception,
      ceremony: s.timelineStages.ceremony[occ],
      cocktail: s.timelineStages.cocktail,
      dinner: s.timelineStages.dinner,
      party: s.timelineStages.party,
      farewell: s.timelineStages.farewell,
    },
    heroTagline: s.heroTagline[occ],
    dressCodeIntro: s.dressCodeIntro,
    scratchSectionTitle: s.scratchSectionTitle,
    scratchInstruction: s.scratchInstruction,
    scratchDayLabel: s.scratchDayLabel,
    scratchMonthLabel: s.scratchMonthLabel,
    scratchYearLabel: s.scratchYearLabel,
    scratchRevealMessage: s.scratchRevealMessage[occ],
    rsvpSubmittingText: s.rsvpSubmittingText,
    rsvpSuccessText: s.rsvpSuccessText,
    rsvpErrorText: s.rsvpErrorText,
    dressCodeTitle: s.dressCodeTitle,
    colorPalette: s.colorPalette,
    ladies: s.ladies,
    gentlemen: s.gentlemen,
    ladiesDesc: s.ladiesDesc,
    gentlemenDesc: s.gentlemenDesc,
    rsvpTitle: s.rsvpTitle,
    rsvpIntro: s.rsvpIntro,
    rsvpButtonText: s.rsvpButtonText,
    rsvpDeadlineNote: s.rsvpDeadlineNote,
    yourNameLabel: s.yourNameLabel,
    willYouComeLabel: s.willYouComeLabel,
    yesOption: s.yesOption,
    noOption: s.noOption,
    foodIntoleranceLabel: s.foodIntoleranceLabel,
    submitButtonText: s.submitButtonText,
    mapTitle: s.mapTitle,
    openInMapsText: s.openInMapsText,
    closingLine: s.closingLine,
    openingTitle: s.openingTitle[occ],
    openingBody: s.openingBody[occ],
    detailsTitle: s.detailsTitle,
    contactIntro: s.contactIntro,
    giftNote: s.giftNote,
  };
}

module.exports = { getStrings, SUPPORTED_LANGUAGES, SUPPORTED_OCCASIONS };
